...
## 后续需求修正：静态目标与只读平台（2026-09-03）

用户已将本轮范围明确为六项修正：静态目标注册/几何、静态融合/遮挡历史、条件误差、新双对象展示、统一只读启动/记录以及对应测试。ADR-013 覆盖本记录历史阶段的目标 CV/速度外推需求；保留无人机实际运动估计。新实现与实际验证见[六项静态目标修正记录](2026-09-03_checklist_static_target_corrections.md)。当前新展示及静态算法已有软件验证，录包回环仍失败；旧入口另有一项异步观测测试未通过。因此尚不声明本轮六项全部完成，也不将历史总体方案的所有后续工作混入本轮完成条件。
