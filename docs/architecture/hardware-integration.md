...
> 赛题范围补充：被定位对象是无人机之外的目标。以下飞控/UWB能力首先是飞行平台硬件基线，不能在未确认标签安装和消息语义前直接当作目标位置。范围决定见 `ADR-002`。
...
ROS2 Humble：目标视觉、坐标求解、算法比较、评估、RViz2、ros2 bag
        ↓
目标世界坐标、指标、图表和演示视频
        └─ 可选：演示需要目标跟踪时再回传 ROS1/飞控
```
...
5. 记录 UWB 标签安装对象，确认数据代表无人机位置、目标位置还是原始观测。
6. 视觉目标位置先离线验证；只有目标跟踪控制需要时才决定是否回传飞控。
