...
- 范围：无人机自身 UWB/视觉惯性定位、机载相机对机外目标的视觉定位，以及两者的坐标链与融合
- 当前决定：`ADR-007`；UWB 定位无人机自身，机载相机定位无人机外目标

## 结论先行

首版不应自研检测器、SLAM 或 UWB 解算器。应先复用以下现有实现并在同一真值、同一时间栅格下比较：

1. **机外目标主链路：AprilTag 3 / `apriltag_ros` + IPPE_SQUARE PnP。** 这是唯一与现有 ROS2 工作区和“机载相机看机外目标”边界直接匹配的低风险方案。
...
4. **后续紧耦合：UVINS 或同类 UWB-VIO。** 只有厂商提供每个基站的原始 UWB 距离、时间戳和基站坐标后才可接入；当前不能用厂商已解算的单一位姿伪装成原始测距输入。
...
- LS、加权 LS、鲁棒 LS、Chan/Taylor 等 UWB 解算只在获得原始距离/TDOA后进入比较；在此之前仅能作为仿真或文献阅读，不能与厂商已解算 UWB 位置作伪对比。

## 建议的复用与比较顺序

1. **先运行 P0。** 以 `apriltag_ros` + IPPE 的机外目标位姿为视觉基线；记录检测率、重投影误差、3D RMSE、P95、可用率和端到端延迟。
2. **并行建立无人机自身基线。** 记录厂商 UWB 位姿；机载图像、IMU 与时间同步满足条件时，再在隔离环境构建 VINS-Fusion 或 ORB-SLAM3，并比较自身位姿误差与漂移。
3. **再比较整链路。** 对同一外部目标真值点、同一 100 ms 栅格比较“视觉相对观测 + 厂商 UWB 位姿变换”与“视觉相对观测 + VIO 位姿变换”。不能将无人机自身误差混入机外目标误差而不单列。
4. **最后决定是否紧耦合。** 只有原始 UWB 距离已开放、P0/P1 失败证据明确且有独立真值时，才引入 UVINS/GTSAM；否则保留可解释的坐标变换或固定协方差融合。
