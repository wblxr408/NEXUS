...
# FanciSwarm reports its UNLOCK state on base_mode bit 0.  This is distinct
# from the standard MAVLink SAFETY_ARMED bit (bit 7).
...
class AutonomousFlight:
    ...
    def _handle(self, message):
        ...
        if typ == "HEARTBEAT":
            ...
            self.last_armed = (bool(self.last_base_mode & FANCISWARM_UNLOCK_FLAG)
                               if self.last_base_mode is not None else None)
            print(
                "HEARTBEAT:",
                "mode=", getattr(message, "base_mode", None),
                ...
                if self.last_fc is not None:
                    if self.last_armed:
                        ...
                        if not math.isfinite(speed) or speed > self.max_speed_mps:
                            print(f"position jump ignored: speed={speed:.2f} m/s")
                            return
                self.last_fc = (position, now)
                if self.origin is None:
                    self.origin = position
                    self.fc_start_time = now
                if (self.fc_start_time is not None and
                        now - self.fc_start_time < FC_STABLE_TIME):
                    return
                else:
    ...
    def ready(self):
        ...
        return (self.last_heartbeat is not None and now - self.last_heartbeat <= HEARTBEAT_MAX_AGE_S and
                self.last_fc is not None and now - self.last_fc[1] <= FC_MAX_AGE_S and
                self.origin is not None and self.fc_start_time is not None and
                now - self.fc_start_time >= FC_STABLE_TIME)
    ...
    def arm(self):
        ...
        self.send_command_no_ack(
            self.protocol.MAV_CMD_COMPONENT_ARM_DISARM,
            (1.0, 0.0),
        )
        ...
        while self._now() < deadline:
            ...
            if self.last_armed:
                print("ARM SUCCESS")
                return
        raise FlightSafetyError("ARM failed")
