...
out=Path('/home/pi/nexus_readonly_runs/2026-09-08_imu_recovery')
...
try:
 ...
 with (out/'role_probe.jsonl').open('x') as f:
  for role,duration in [('gcs',10),('onboard',20),('gcs_restore',10)]:
   start=time.monotonic();next_hb=0;counts=Counter()
   while time.monotonic()-start<duration:
    elapsed=time.monotonic()-start
    if elapsed>=next_hb:
     master.mav.heartbeat_send(18 if role=='onboard' else 6,8,0,0,4)
     next_hb=elapsed+1
    ...
    f.write(json.dumps(dict(role=role,elapsed_s=elapsed,receive_unix_ns=time.time_ns(),message=msg.to_dict()))+'\n')
