...
try:
 ...
 with (out/'motion.jsonl').open('x') as f:
  for role,duration in [('awaiting_motion',1200)]:
   start=time.monotonic();next_hb=0;counts=Counter()
   while time.monotonic()-start<duration and not (out/'stop_motion').exists():
    role=(out/'phase.txt').read_text().strip()
