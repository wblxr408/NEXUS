...
def test_platform_restart_recovers_without_restarting_sensor_pipeline(tmp_path):
    ...
    outputs = []
    events = []
    driver.create_subscription(String, "/nexus/platform/initialization_status", lambda msg: events.append(json.loads(msg.data)), 20)
    ...
    def wait(predicate):
        ...
        assert predicate(), str(dict(events=events[-8:], request=initializer.request_id, initialized=initializer.initialized, samples=len(initializer.samples), minimum=initializer.minimum_stamp, reason=platform._initialization_reason, states=len(platform.estimator.states)))
