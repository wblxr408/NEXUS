    ...
    for name in ALGORITHMS:
        if name in ("uwb.matlab.ekf", "uwb.matlab.ukf"):
            ...
            if hasattr(runner, "_state"):
                del runner._state
        result = AlgorithmRouter(name).run(observation_frame=frames[0])
        assert result.valid, f"{name} failed: {result.metadata}"
        error = np.linalg.norm(result.estimate - np.array(gt[:2]))
        assert error < 1e-6, f"{name} static error={error}"
...
def test_trajectory_all_algorithms():
    ...
    for name in ALGORITHMS:
        ...
        filter_thresholds = {"uwb.matlab.taylor": 0.2, "uwb.matlab.ekf": 0.15, "uwb.matlab.ukf": 0.25}
