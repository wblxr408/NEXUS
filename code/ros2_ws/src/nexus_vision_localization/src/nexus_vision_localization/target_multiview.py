...
The target is fixed in map; platform motion is measured, never commanded.
Platform and mounting uncertainty are neglected by the current experiment.
Returned covariance is conditional on those poses; pixel noise is retained.
...
class MultiviewEstimate:
    ...
    covariance: np.ndarray  # 3x3 position, conditional on platform and mounting poses
    inlier_stamps: tuple
...
def _prepared(observation, extrinsic):
    camera = observation.platform.transform @ extrinsic
    ...
    return camera, measured, pixel_covariance
...
def solve_multiview(observations, transform_body_camera, extrinsic_covariance, *, motion_model="static", config=None):
    ...
    dimension = 3
    minimum = max(config.minimum_views, 2)
    ...
    # Retain calibration validation, without propagating this covariance.
    pose_covariance(extrinsic_covariance)
    ...
    linear, rhs = [], []
    for observation, (_, measured, _), rotation, center in zip(observations, prepared, rotations, centers):
        ray = np.linalg.solve(observation.camera_matrix, np.r_[measured, 1.])
        projection = np.array([[1., 0., -ray[0]], [0., 1., -ray[1]]]) @ rotation.T
        linear.append(projection)
    ...
    def project(state, index, derivatives=False):
        ...
        state_jacobian = derivative @ rotations[index].T
        return prediction, z, state_jacobian

    def consensus(state):
        errors, keep = [], []
        for index, (_, measured, _) in enumerate(prepared):
    ...
    for _ in range(2):
        ...
        for index in indices:
            noise = prepared[index][2]
            whitening.append(np.linalg.solve(np.linalg.cholesky(noise), np.eye(2)))

        def residual(value):
            return np.concatenate([w @ (project(value, index)[0] - prepared[index][1]) for index, w in zip(indices, whitening)])
    ...
    rays = [state - centers[index] for index in indices]
    ...
    jacobians, weights = [], []
    for index in indices:
        predicted, _, j_state = project(state, index, True)
        noise = prepared[index][2]
        weight = np.linalg.solve(noise, np.eye(2))
        distance = np.sqrt((predicted - prepared[index][1]) @ weight @ (predicted - prepared[index][1]))
        weight *= min(1., 1.5 / max(distance, 1e-12))
        information += j_state.T @ weight @ j_state
        jacobians.append(j_state)
    ...
    covariance = np.zeros_like(inverse)
    for index, j_state, weight in zip(indices, jacobians, weights):
        gain = inverse @ j_state.T @ weight
        covariance += gain @ prepared[index][2] @ gain.T
    return MultiviewEstimate(stamps[-1], state.copy(), None,
                             (covariance + covariance.T) / 2, tuple(stamps[index] for index in indices),
                             float(np.sqrt(np.mean(errors[indices] ** 2))), parallax, condition, "static")
