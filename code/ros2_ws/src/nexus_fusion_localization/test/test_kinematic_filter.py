...
def observation(stamp=1_000_000_000, position=(1., 2., 3.), velocity=(.2, -.1, 0.), rotation=None,
                covariance=None, reference="reference_feature:test:0", source="superpoint_multiview_constant_velocity", target_id="one"):
...
def test_ci_matches_independent_information_formula_and_keeps_pv_cross_terms():
    ...
    left = observation(covariance=(left_root @ left_root.T + np.eye(6)) * .01)
...
def test_map_angle_covariance_conversion_matches_independent_rotation_differences():
    origin = observation(rotation=Rotation.from_euler("xyz", [.3, -.2, .4]).as_matrix())
    rotation = Rotation.from_rotvec([.15, -.25, .1]).as_matrix() @ origin.rotation
    rng = np.random.default_rng(91)
    root = rng.normal(size=(9, 9)) * .02
    item = observation(rotation=rotation, covariance=root @ root.T + np.eye(9) * .001)
...
def test_partial_observations_add_only_observed_components_and_preserve_existing_cross_blocks():
    ...
    second = observation(rotation=np.eye(3), covariance=np.eye(9) * .02)
...
def test_prediction_uses_velocity_cross_covariance_but_never_becomes_observed():
    estimator = KinematicTargetFilter()
    covariance = np.eye(6) * .04
    covariance[:3, 3:] = covariance[3:, :3] = np.eye(3) * .01
    first = observation(covariance=covariance)
    assert estimator.update(first, first.stamp_ns) is not None
    future = first.stamp_ns + 300_000_000
    prediction = estimator.predict(first.target_id, future)
    assert prediction["valid"] is False and prediction["last_observed_stamp_ns"] == first.stamp_ns
    np.testing.assert_allclose(prediction["estimate"].position, first.position + .3 * first.velocity)
    f = np.eye(6)
    f[:3, 3:] = np.eye(3) * .3
    g = np.vstack((np.eye(3) * .3 ** 2 / 2, np.eye(3) * .3))
    expected = f @ covariance @ f.T + g @ g.T * estimator.config.acceleration_sigma_mps2 ** 2
    np.testing.assert_allclose(prediction["estimate"].covariance[:6, :6], expected)
    assert estimator.tracks[first.target_id].estimate.stamp_ns == first.stamp_ns
    assert estimator.predict(first.target_id, first.stamp_ns + 500_000_001) is None
...
def test_hard_outlier_cannot_be_rescued_by_learned_inflation_and_valid_data_recovers():
    ...
    assert estimator.tracks[first.target_id].estimate.stamp_ns == first.stamp_ns
    good = observation(first.stamp_ns + 20_000_000, position=first.position + first.velocity * .02)
...
def test_recovery_requires_new_consistent_evidence_without_old_location_tie_break(reason):
    estimator = KinematicTargetFilter()
    first = observation()
    estimator.update(first, first.stamp_ns)
    ...
    if reason == "identity_ambiguous":
        ...
        assert estimator.predict(first.target_id, start) is None
    reference = "reference_feature:changed:2" if reason == "reference_change" else first.position_reference
    for i in range(3):
        item = observation(start + (i + 1) * 10_000_000, position=(9. + .002 * i, 2., 3.), reference=reference)
        ...
        if i < 2:
            ...
            assert estimator.predict(first.target_id, item.stamp_ns) is None
        else:
...
def test_future_old_duplicate_and_invalid_component_inputs_never_replace_current_state():
    ...
    assert estimator.predict(first.target_id, first.stamp_ns + 20_000_000) is not None
...
def test_stale_optional_components_expire_even_with_continuing_position_updates():
    ...
    for step in range(1, 5):
        item = observation(first.stamp_ns + step * 150_000_000, position=first.position + first.velocity * step * .15, velocity=None)
        result = estimator.update(item, item.stamp_ns)
...
def test_target_and_source_memory_is_bounded_and_expired_tracks_are_removed():
    ...
    assert not estimator.tracks and not estimator.source_stamps and not estimator.recovery
