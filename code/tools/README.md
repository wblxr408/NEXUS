...
`hardware_acceptance_capture.py` 用于到货首日的只读证据采集：保存 `rostopic list/type/echo -n 1`、ROS/Git 版本、主机时间同步状态、SD CSV 表头/大小/SHA256，以及明确保留 `TBD` 的 `metadata_draft.json`。它不复制 rosbag、SD 文件正文、设备序列号或网络凭据。完整命令和产物说明见 [硬件验收证据采集](../../docs/2026-08-22_guide_hardware_acceptance_evidence_capture.md)；采集结果再按 [当前执行基线](../../docs/2026-08-25_plan_current_execution_baseline.md) 归档到对应运行目录。
